...
After conducting Levene's test of homogeneity of variance, we see that our p-value of 0.5326 is greater than 0.05. This means that we do not have statistically significant evidence that we have different variances, therefore constant variance condition is not violated. (**double check)\*\*\*\*\***
...
mse <- 1.6888 
SD <- sqrt(1.6888)
effectsize <- 1.71 - 1.5 / SD
#0.555744
```

The output above shows us descriptive statistics of our milk data, and we can use it to calculate our effect size. To calculate effect size, we use the DairyPro (1.71) and DairyCon (1.5). This means \| 1.71-1.5\| are divided by rad(1.6888) = 0.16. This represents a trivial effect based on Cohen's table. (**double check)\*\*\*\*\***
