...
milk_summary
## DO WE NEED THIS IF WE ARE DOING LEVENS TEST? BUT HERE IT SAYS SD ARE TOO DIFFERENT BUT LEVENS SAYS WE FAIL TO REJECT NULL HYPO
```
...
Our largest standard deviation \[Dairy_control group\] 1.75 divided by the smallest standard deviation \[PlantPro group\] 0.577 equals 3.032. Their quotient is larger than 2, so we violate the condition about constant variance for our ANOVA model.
...
#BONFERONNI
milk_bonf <- 
  bonfCI(milk_df1$average_consumption, interaction(milk_df1$milk_type,milk_df1$ad_type),
       #  milk_df1$treatment,
         conf.level = 0.95)
