...
After condicting Levene's test of homogeneity of variance, we see that our p-value of 0.5326 is greater than 0.05. This means that we do not have statistically significant evidence that we have different variances, therefor our condition is not violated.
...
When conducting a pairwise comparison, a fishers lsd looks at the individual false alarm rate. This means it looks at the chance of a Type 1 error for a single test. Because our alpha level is 0.05, this means that for every error, those 0.05 error rates add up. The Fishers LSD is the least conservative, meaning it is likely to reject the null hypothesis that the means of average consumption are the same.

When looking at our pairwise comparisons, we see that all of the p-values are larger than 0.05. This means we do not have statistically significant evidence that the pairwise comparisons between the treatments are different.
...
The Bonferroni confidence intervals focuses on family wise fale alarm rate. Meaning we look at the 0.05 error on *all* the tests we conduct. This approach is the most conservative and least likely to reject the null hypothesis.

Again, based on all of our pairwise comparisons, all of the p-values are larger than 0.05. This means we do not have statistically significant evidence that the pairwise comparisons between the treatments are different.
...
The TukeyHSD is the last pairwise comparison test that we will look at. It is somewhere in between the fishers and bonferonni. Similarly, when looking at our pairwise comparisons, we see that all of the p-values are larger than 0.05. We do not have statistically significant evidence that the pairwise comparisons between the treatments are different.
...
The output above shows us descriptive statistics of our milk data. We can use it to calculate our effect size. To calculate effect size, we \| 1.71-1.5\|/ rad(1.6888) = 0.16. This represents a trivial effect.
