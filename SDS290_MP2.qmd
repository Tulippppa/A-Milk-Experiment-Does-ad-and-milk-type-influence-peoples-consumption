...
```{r, LOADPKGE, warning=F, message=F}
library(tidyverse)
library(rio)
library(asbio)
library(dplyr)
...
The bar chart demonstrates that there were more participants that are not lactose intolerant, than lactose intolerant. Therefore, although we accounted for this confounding variable pre-survey, the responses indicate that being lactose intolerant did not significantly impact our responses due less than half of students identifying with this.

```{r, LACTOSEINTOLERANT-PLOT, warning=F, message=F, fig.height = 3, fig.width = 6, fig.align = "center", echo=FALSE}
lactose_plot  <- 
  ggplot(data=milk_df1, aes(x=lactose_intolerant, y=average_consumption)) + 
...
The second bar chart shows that all students that responded to the survey were not vegan, so this is not a significant confounding variable impacting our results.
...
The interaction model demonstrates that we do not have statistically significant evidence that the effect of ad type on the mean rate of participant's consumption is different based on the effect of milk type, and vice versa. Additionally, the effect of milk type on the mean rate of participant's consumption is not different based on the effect of ad type. We do not have statistically significant evidence that an interaction exists between the ad type and milk type.
