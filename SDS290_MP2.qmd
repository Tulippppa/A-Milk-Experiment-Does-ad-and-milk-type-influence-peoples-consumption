...
milk_summary <- 
  milk_df1 |>
  group_by(treatment) |>
  summarise(group_mean=mean(average_purchase), 
            group_n=n(), 
...
milk_means <-
  ggplot(data=milk_df1, aes(x=average_purchase, y=treatment)) + 
...
[**Condition Check 3 Additive Effects**]{.underline}:

The Tukey Non-Additivity plot model helps us understand the relationship between the affect of both treatments over what we predicted. It also allows us to assess if the affects of treatments are additive or if there is some interaction. We want to relationship between the x-axis and y-axis to have a slope of 0 to indicate there is no relationship. However, the slope is 2.7, so we must transform the data.
...
TukeyNonaddPlot (average_purchase ~ ad_type + milk_type, data=milk_df1)
```

[**Conditions Interpretation & Transformation:**]{.underline}

Our condition assumptions indicate at least one violation, so we must transform our data.
...
log_milk <- 
  milk_df1 |>
  group_by(treatment) |>
  summarise(log_mean=log(mean(average_purchase)), 
            group_n=n(), 
            log_sd=log(sd(average_purchase))) 
